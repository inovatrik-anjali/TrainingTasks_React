...
export default function ImageCarousel() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setIndex((i) => (i < images.length - 1 ? i + 1 : i)); // stop at last
    }, 3000);
    ...
    <>
      ...
      >
        ...
        <button
          ...
          disabled={index === 0}
          className={`absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full shadow transition ${
            index === 0
        ...
        <button
          ...
          className={`absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full shadow transition ${
